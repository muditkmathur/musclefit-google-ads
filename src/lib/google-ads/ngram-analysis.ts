...
function tokenize(text: string, cfg: Pick<ResolvedOptions, "minTokenLen" | "keepNumbers" | "keepStopwords">): string[] {
  if (!text) return [];
  return text
    .split(/\s+/g)
    .filter(Boolean)
...
function getWeight(row: Partial<SearchTermRow>, weight: ResolvedOptions["weight"]): number {
  switch (weight) {
    case "clicks":
...
export function analyzeNgrams({ rows, options = {} }: AnalyzeNgramInput): NgramAnalysisResult {
  ...
  for (const n of opts.n) {
    const sorted = sortEntries(ngramMaps.get(n)!).filter((x) => x.count >= opts.minCount);
...
export const DEFAULT_NGRAM_OUTPUT = join(process.cwd(), "data", "ngram-analysis.json");
...
export async function runNgramAnalysisFromFile(options: RunNgramFromFileOptions = {}): Promise<NgramAnalysisResult> {
  ...
  if (!rows.length) {
    throw new Error(`No rows found in ${inputPath}. Expected { rows: [...] } from search-terms output.`);
  }
